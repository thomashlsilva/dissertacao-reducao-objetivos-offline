...
def compute_igd(n_obj, n_obj_red):
    ...
    for j in range(21):
        ...
        pf_list.append(
            pd.read_csv(f"./dtlz1_elim/pareto_front/{n_obj}obj/{n_obj_red}red_pf{j}.csv", header=None).to_numpy())
        igd.append(ind(pf_list[j]))

    igd_arr = np.array(igd)
    pd.DataFrame(igd_arr).to_csv(f"dtlz1_elim/igd_plus/{n_obj}obj_{n_obj_red}red_igd_plus.csv",
