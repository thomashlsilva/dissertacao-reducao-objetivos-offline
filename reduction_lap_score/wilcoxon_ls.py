...
def main():
    ...
    sys.stdout = open(f'./dtlz{number_dtlz}_sum/wilcoxon_results/{n_obj}obj.txt', 'w')

    # IGD+ vector from original problem
    igd_orig = pd.read_csv(f"./dtlz{number_dtlz}_sum/igd_plus/{n_obj}obj_{n_obj}red_igd_plus.csv",
                           header=None).to_numpy()
    # Epsilon-indicator vector from original problem
    eind_orig = pd.read_csv(f"./dtlz{number_dtlz}_sum/eps_ind/{n_obj}obj_{n_obj}red_eind.csv",
                            header=None).to_numpy()
    # Hyper-volume vector from original problem
    # hv_orig = pd.read_csv(f"./dtlz1_elim/hv/{n_obj}obj_{n_obj}red_hvwfg.csv",
    #                       header=None).to_numpy()

    for n_obj_red in n_obj_red_list:
        print('\nNumber of objectives:', n_obj_red)
        # Read IGD+ for all cases
        igd_arr = pd.read_csv(f"./dtlz{number_dtlz}_sum/igd_plus/{n_obj}obj_{n_obj_red}red_igd_plus.csv",
                              header=None).to_numpy()
        # Read epsilon-indicator for all cases
        eind_arr = pd.read_csv(f"./dtlz{number_dtlz}_sum/eps_ind/{n_obj}obj_{n_obj_red}red_eind.csv",
                               header=None).to_numpy()
        # Read hyper-volume for all cases
        # hv_arr = pd.read_csv(f"./dtlz1_elim/hv/{n_obj}obj_{n_obj_red}red_hvwfg.csv", header=None).to_numpy()
        ...
        print('\n')
        # Hyper-volume
        print('Hyper-volume Reduced mean: ', np.mean(hv_arr))
        print('- Wilcoxon rank-elim test: ',
              stats.rankelims(hv_arr, hv_orig, alternative='two-sided', axis=0))
              
        if np.array_equal(hv_arr, hv_orig):
            print("eind_arr and eind_orig are identical.")
        else:
            print("eind_arr and eind_orig are not identical.")
        """
